...
static inline bool is_nonempty(const std::string& s)
{
    return !s.empty();
}
...
bool load_yaml_config(const std::string& yaml_path, ExtractorConfig& out, std::string& err)
{
    try
    {
        ...
        if (auto n = root["processing"]; n && n.IsMap())
        {
            ...
            if (auto v = n["feature_normalization"]; v && v.IsScalar())
            {
                ...
                out.feature_normalization.mode =
                    (s == "grid") ? FeatureNormalizationMode::Grid : FeatureNormalizationMode::None;
            }
            ...
            if (auto gn = n["grid_normalization"]; gn && gn.IsMap())
            {
                ...
                if (auto sc = gn["score"]; sc && sc.IsScalar())
                {
                    ...
                    out.feature_normalization.grid.score =
                        (ss == "size") ? GridNormalizationParams::Score::Size
        ...
        // Determine base_dir: prefer output.base_dir, else inferred from any set path, else
        // "output"
        ...
        // We still delegate parsing to core to avoid duplicating schema rules. For that we use the
        // parseEnhanceConfig function.
        std::string enhance_err;
        auto enhance_cfg = videostrip::parseEnhanceConfig(
            root, enhance_err); // return type std::optional<EnhanceConfig>
        ...
        if (out.apply_enhancement && !root["enhance"])
        {
            ...
            out.enhance.sequence.push_back(
                {EnhanceType::CLAHE, ClaheParams{2.0, {8, 8}, ClaheSpace::YCrCb}});
        }
...
void normalize_output_paths(ExtractorConfig& cfg, const std::string& base_dir)
{
    ...
    if (!is_nonempty(cfg.output_images_dir) && !is_nonempty(cfg.output_features_dir) &&
        !is_nonempty(cfg.output_metadata_csv) && !is_nonempty(cfg.output_summary_yaml) &&
...
void merge_yaml_into(ExtractorConfig& dst, const ExtractorConfig& y)
{
    ...
    if (!y.feature_type.empty())
        dst.feature_type = y.feature_type;
    if (!y.image_format.empty())
        dst.image_format = y.image_format;

    // Numeric/bool fields: copy directly (assume src had meaningful values when set)
    // We can’t distinguish “unset” from “default” here; rely on loader side to only set when key
    // exists.
