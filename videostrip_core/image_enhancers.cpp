...
void Enhancer::clear() { sequence_.clear(); }
    ...
    for (const auto& step : sequence_) {
        switch (step.type) {
            case EnhanceType::ContrastOffset:
                if (!opContrastOffset(bgr, std::get<ContrastOffsetParams>(step.params))) return false;
                break;
            case EnhanceType::GrayWorldWB:
                if (!opGrayWorld(bgr, std::get<GrayWorldParams>(step.params))) return false;
                break;
            case EnhanceType::Gamma:
                if (!opGamma(bgr, std::get<GammaParams>(step.params))) return false;
                break;
            case EnhanceType::CLAHE:
                if (!opClahe(bgr, std::get<ClaheParams>(step.params))) return false;
                break;
...
std::vector<EnhanceStep> Enhancer::parseSequence(const std::string& spec) {
    ...
    while (std::getline(ss, token, ';')) {
        ...
        std::string args = (lparen != std::string::npos && rparen != std::string::npos && rparen > lparen)
                         ? token.substr(lparen + 1, rparen - lparen - 1) : "";

        // normalize
        std::string lname; lname.resize(name.size());
        std::transform(name.begin(), name.end(), lname.begin(), [](unsigned char c){ return std::tolower(c); });
        ...
        auto parseKey = [&](const std::string& key, const std::string& def="") -> std::string {
            ...
            std::string v = args.substr(pos, end == std::string::npos ? std::string::npos : end - pos);
            // trim spaces
        ...
        if (lname.rfind("contrast", 0) == 0) {
            ...
            if (!args.empty()) {
                if (auto s = parseKey("alpha"); !s.empty()) p.alpha = std::stod(s);
                if (auto s = parseKey("beta");  !s.empty()) p.beta  = std::stod(s);
            }
            ...
            if (!args.empty()) {
                ...
                try { p.gamma = std::stod(args); } catch (...) {
                    if (auto s = parseKey("value"); !s.empty()) p.gamma = std::stod(s);
                }
            }
            ...
            ClaheParams p;
            if (!args.empty()) {
                if (auto s = parseKey("clip"); !s.empty()) p.clipLimit = std::stod(s);
                ...
                if (auto s = parseKey("space"); !s.empty()) {
                    std::string ls; ls.resize(s.size());
                    std::transform(s.begin(), s.end(), ls.begin(), [](unsigned char c){ return std::tolower(c); });
...
bool Enhancer::opGrayWorld(cv::Mat& bgr, const GrayWorldParams&) {
    ...
    if (mB <= 1e-9 || mG <= 1e-9 || mR <= 1e-9) return true; // avoid blow-ups

    double sB = mGlobal / mB;
    double sG = mGlobal / mG;
    double sR = mGlobal / mR;

    std::vector<cv::Mat> ch; ch.reserve(3);
    cv::split(bgr, ch);
...
void Enhancer::ensureClahe(const ClaheParams& p) {
    if (!clahe_ || !claheValid_
        || p.clipLimit != cachedClaheParams_.clipLimit
...
static inline void convertBGRTo(const cv::Mat& bgr, cv::Mat& out, ClaheSpace s) {
    switch (s) {
        case ClaheSpace::YCrCb: cv::cvtColor(bgr, out, cv::COLOR_BGR2YCrCb); break;
        case ClaheSpace::HSV:   cv::cvtColor(bgr, out, cv::COLOR_BGR2HSV);   break;
        case ClaheSpace::Lab:   cv::cvtColor(bgr, out, cv::COLOR_BGR2Lab);   break;
        case ClaheSpace::BGR:   out = bgr; /* shallow copy ok */             break;
    }
}
static inline void convertToBGR(const cv::Mat& in, cv::Mat& bgr, ClaheSpace s) {
    switch (s) {
        case ClaheSpace::YCrCb: cv::cvtColor(in, bgr, cv::COLOR_YCrCb2BGR); break;
        case ClaheSpace::HSV:   cv::cvtColor(in, bgr, cv::COLOR_HSV2BGR);   break;
        case ClaheSpace::Lab:   cv::cvtColor(in, bgr, cv::COLOR_Lab2BGR);   break;
        case ClaheSpace::BGR:   bgr = in; /* shallow copy ok */             break;
    }
}
...
bool Enhancer::opClahe(cv::Mat& bgr, const ClaheParams& p) {
    ...
    if (p.space == ClaheSpace::BGR) {
        ...
        std::vector<cv::Mat> ch; ch.reserve(3);
        cv::split(bgr, ch);
        for (auto& c : ch) clahe_->apply(c, c);
        cv::merge(ch, bgr);
        return true;
    }
    ...
    int idx = 0;
    if (p.space == ClaheSpace::HSV) idx = 2; // H,S,V
    else if (p.space == ClaheSpace::YCrCb) idx = 0; // Y,Cr,Cb
    else if (p.space == ClaheSpace::Lab) idx = 0; // L,a,b
