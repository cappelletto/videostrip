...
std::optional<EnhanceConfig> parseEnhanceConfig(const YAML::Node& root, std::string& err)
{
    ...
    for (const auto& item : seq)
    {
        if (!item || !item.IsMap())
        {
            err = "enhance.sequence entry must be a map";
            return std::nullopt;
        }
        auto tnode = item["type"];
        if (!tnode)
        {
            err = "enhance.sequence entry missing 'type'";
            return std::nullopt;
        }
        ...
        if (t == "contrast" || t == "contrastoffset")
        {
            ...
            if (auto a = item["alpha"])
                p.alpha = a.as<double>(1.0);
            if (auto b = item["beta"])
                p.beta = b.as<double>(0.0);
        ...
        else if (t == "gamma")
        {
            ...
            if (item["value"])
                p.gamma = item["value"].as<double>(1.0);
        ...
        {
            ClaheParams p{};
            if (auto c = item["clip_limit"])
                p.clipLimit = c.as<double>(2.0);
