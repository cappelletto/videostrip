...
namespace videostrip
{
    ...
    bool VideoFrameExtractor::run()
    {
        ...
        while (cap.read(frame_bgr)) {
            ...
            if (overlap == -2.0f) { // legacy: force new keyframe search
            ...
            if (overlap <= static_cast<float>(m_config.overlap_threshold)) {
                ...
                for (int n = 1; n < kWindow; ++n) {
                    cv::Mat cand_bgr;
                    if (!cap.read(cand_bgr)) break; // end of stream
                    ++frame_idx;
                    ...
                    if (m_progress_cb) {
                        float prog = (total_frames > 0) ? (static_cast<float>(frame_idx) / static_cast<float>(total_frames)) : 0.0f;
                        std::ostringstream msg; msg << "Refining (" << n + 1 << "/" << kWindow << ")";
        ...
        m_summary.total_frames = total_frames; // retrieved from video file property header (CAP_PROP_FRAME_COUNT)
