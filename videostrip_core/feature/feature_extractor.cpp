...
void write_feature_file(const std::string& path, const std::vector<cv::KeyPoint>& kpts)
{
    ...
    {
        ofs << k.pt.x << " " << k.pt.y << " " << k.size << " " << k.angle << " " << k.response
            << " " << k.octave << " " << k.class_id << "\n";
    }
}
...
{
    if (norm_cfg_.mode != FeatureNormalizationMode::Grid)
        return;
    std::cout << "Applying GRID normalization" << std::endl;
    const auto& gp = norm_cfg_.grid;
    if (gp.cell_w <= 0 || gp.cell_h <= 0 || gp.max_per_cell <= 0)
        return;
    if (kpts.empty())
        return;
    ...
    auto score_of = [&](const cv::KeyPoint& kp) -> float
    {
        switch (gp.score)
        {
        case GridNormalizationParams::Score::Size:
            return kp.size;
...
{
public:
    ORBFeatureExtractor() : orb_(cv::ORB::create()) {}
    ...
    int extract(const std::string& image_path, std::string& feature_file_out,
...
public:
    AKAZEFeatureExtractor()
        : akaze_(cv::AKAZE::create(cv::AKAZE::DESCRIPTOR_MLDB, // descriptor_type
...
public:
    SURFFeatureExtractor()
        : surf_(
              cv::xfeatures2d::SURF::create(400.0, // hessianThreshold
                                            4,     // nOctaves
                                            3,     // nOctaveLayers
